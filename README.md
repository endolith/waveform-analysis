...
* [SoundFile](https://python-soundfile.readthedocs.io/) for opening any file format supported by [libsndfile](http://www.mega-nerd.com/libsndfile/) (including MP3).  Otherwise, it falls back to SciPy's very limited WAV file support.
  * (Note to self, updated 2026-08-19: the conda-forge package is named `pysoundfile` and is current (v0.14.0, same bastibe/python-soundfile repo) — `conda install -c conda-forge pysoundfile` works fine.  PyPI name is `soundfile` (`pip install soundfile`).)
